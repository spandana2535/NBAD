...
exports.profile = (req, res, next) => {
  ...
  Promise.all([
    user.findById(id),
    model.find({ CreatedBy: id }),
    save_model.find({ SavedBy: id }),
    offer_model.find({ OfferedBy: id }),
  ])
    .then((results) => {
      const [user, items, saves, offers] = results;
      res.render("./user/profile", { user, items, saves, offers });
    })
    .catch((err) => {
      next(err);
    });
  ...
  offer_model
    ...
    .then((offer1) => {
      ...
      Promise.all([
        ...
        offer_model.findByIdAndDelete(id, { useFindAndModify: false }),
      ])
        .then((results) => {
