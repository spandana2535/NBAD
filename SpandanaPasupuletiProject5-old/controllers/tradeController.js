...
exports.update = (req, res, next) => {
  ...
  model
    ...
    .then((item) => {
      req.flash("success", "Succesfylly updated an item");
    ...
    .then((item) => {
      ...
      Promise.all([
        ...
        .then((results) => {
          ...
          console.log("---115---offer is" + offer);
          console.log("---116---item is" + item);
        })
        .catch((err) => {
          next(err);
        });
...
exports.cart = (req, res, next) => {
  ...
  model
    ...
    .then((item) => {
      ...
      console.log("new cart item  is :" + newCartItem);
          ...
          if (!item) {
            newSaveItem
              ...
              .then((newSaveItem) => {
                ...
                model
                  .findByIdAndUpdate(id, { Saved: true },  {
                    useFindAndModify: false,
                    runValidators: true,
                  })
                  .then((item) => {
...
exports.trade = (req, res, next) => {
  ...
  model
    ...
    .then((item) => {
      ...
      Promise.all([
        newOfferItem.save(),
        model.findByIdAndUpdate(iD, { Status: "Offer Pending" },  {
          useFindAndModify: false,
          runValidators: true,
        }),
      ]).then((results) => {
        const [offer, item] = results;
        console.log("offer item is " + newOfferItem);
        console.log("trade item is " + item);
        model
          .find({ CreatedBy: user })
          .then((items) => {
  ...
  model
    ...
    .then((item) => {
      ...
      Promise.all([
        ...
        offer_model.findOneAndDelete({ Name: name },{ useFindAndModify: false }),
      ])
        .then((results) => {
